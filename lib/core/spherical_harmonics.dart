...
int SHTerms(int lmax) => (lmax + 1) * (lmax + 1);
...
void SHEvaluate(Vector w, int lmax, List<double> out, [int outIndex = 0]) {
  if (lmax > 28) {
      LogSevere(
          "SHEvaluate() runs out of numerical precision for lmax > 28. "
  ...
  List<double> Klm = new List<double>(SHTerms(lmax));

  for (int l = 0; l <= lmax; ++l) {
    for (int m = -l; m <= l; ++m) {
      Klm[SHIndex(l, m)] = _K(l, m);
    }
  }
  ...
  for (int l = 0; l <= lmax; ++l) {
    for (int m = -l; m < 0; ++m) {
      ...
      assert(out[SHIndex(l,m)].isNaN);
      assert(out[SHIndex(l,m)].isFinite);
    }
...
void SHProjectCube(func, Point p, int res, int lmax,
  ...
  for (int u = 0; u < res; ++u) {
    ...
    for (int v = 0; v < res; ++v) {
      ...
      SHEvaluate(Vector.Normalize(w), lmax, Ylm);
...
void SHProjectIncidentDirectRadiance(Point p, double pEpsilon, double time,
    Scene scene, bool computeLightVisibility, int lmax, RNG rng,
    List<Spectrum> c_d) {
  // Loop over light sources and sum their SH coefficients
  List<Spectrum> c = new List<Spectrum>(SHTerms(lmax));
...
void SHProjectIncidentIndirectRadiance(Point p, double pEpsilon,
    double time, Renderer renderer, Sample origSample,
    Scene scene, int lmax, RNG rng, int nSamples,
    List<Spectrum> c_i) {
...
void SHReduceRinging(List<Spectrum> c, int lmax, [double lambda = 0.005]) {
  for (int l = 0; l <= lmax; ++l) {
    double scale = 1.0 / (1.0 + lambda * l * l * (l + 1) * (l + 1));
    for (int m = -l; m <= l; ++m) {
      c[SHIndex(l, m)].scale(scale);
    }
  }
}

void SHRotate(List<Spectrum> c_in, List<Spectrum> c_out, Matrix4x4 m,
              int lmax) {
  ...
  List<Spectrum> work = Spectrum.AllocateList(SHTerms(lmax));

  SHRotateZ(c_in, c_out, gamma[0], lmax);
  SHRotateXPlus(c_out, work, lmax);
  SHRotateZ(work, c_out, beta[0], lmax);
  SHRotateXMinus(c_out, work, lmax);
  SHRotateZ(work, c_out, alpha[0], lmax);
}

void SHRotateZ(List<Spectrum> c_in, List<Spectrum> c_out, double alpha,
               int lmax) {
  //Assert(c_in != c_out);
  c_out[0].copy(c_in[0]);
  if (lmax == 0) {
  ...
  for (int l = 1; l <= lmax; ++l) {
    ...
    for (int m = -l; m < 0; ++m) {
      c_out[SHIndex(l, m)] = c_in[SHIndex(l,  m)] * ct[-m] +
                             c_in[SHIndex(l, -m)] * -st[-m];
    }

    c_out[SHIndex(l, 0)].copy(c_in[SHIndex(l, 0)]);
    for (int m = 1; m <= l; ++m) {
      c_out[SHIndex(l, m)] = c_in[SHIndex(l,  m)] * ct[m] +
                             c_in[SHIndex(l, -m)] * st[m];
    }
...
void SHRotateXMinus(List<Spectrum> c_in, List<Spectrum> c_out,
                    int lmax) {
  ...
  for (int l = 1; l <= lmax; ++l) {
    ...
    c_out[SHIndex(l, 0)].scale(s);
    for (int m = 1; m <= l; ++m) {
      s = -s;
      c_out[SHIndex(l, m)].scale(s);
      c_out[SHIndex(l, -m)].scale(-s);
    }
  }
}

void SHRotateXPlus(List<Spectrum> c_in, List<Spectrum> c_out, int lmax) {
  O(l, m) => c_in[SHIndex(l, m)];
  int oi = 0;
  ...
  c_out[oi++] = (O(3, 0) * -0.7905694150420949 + O(3, 2) * 0.6123724356957945);
  c_out[oi++] = -O(3,-2);
  c_out[oi++] = (O(3, 0) * -0.6123724356957945 + O(3,2) * -0.7905694150420949);
  c_out[oi++] = (O(3, -3) * 0.7905694150420949 + O(3, -1) * 0.6123724356957945);
  c_out[oi++] = (O(3, 1) * -0.25 + O(3, 3) * -0.9682458365518543);
  c_out[oi++] = (O(3, -3) * -0.6123724356957945 + O(3, -1) * 0.7905694150420949);
  ...
  c_out[oi++] = O(4, 1) * -0.35355339059327373 + O(4, 3) * -0.9354143466934853;
  c_out[oi++] = O(4, -3) * 0.6614378277661477 + O(4, -1) * 0.75;
  c_out[oi++] = O(4, 0) * 0.375 + O(4, 2) * 0.5590169943749475 + O(4, 4) * 0.739509972887452;
  c_out[oi++] = O(4, -4) * 0.9354143466934853 + O(4, -2) * 0.35355339059327373;
  c_out[oi++] = O(4, 0) * 0.5590169943749475 + O(4, 2) * 0.5 + O(4, 4) * -0.6614378277661477;
  c_out[oi++] = O(4, -4) * -0.35355339059327373 + O(4, -2) * 0.9354143466934853;
  c_out[oi++] = O(4, 0) * 0.739509972887452 + O(4, 2) * -0.6614378277661477 + O(4, 4) * 0.125;
  ...
  c_out[oi++] = (O(5,1) * 0.125 + O(5,3) * 0.4050462936504913 + O(5,5) * 0.9057110466368399);
  c_out[oi++] = (O(5,-5) * 0.6846531968814576 - O(5,-3) * 0.30618621784789724 -
              O(5,-1) * 0.6614378277661477);
  c_out[oi++] = (O(5,1) * 0.4050462936504913 + O(5,3) * 0.8125 - O(5,5) * 0.4192627457812106);
  c_out[oi++] = (O(5,-5) * -0.19764235376052372 + O(5,-3) * 0.795495128834866 -
              O(5,-1) * 0.57282196186948);
  c_out[oi++] = (O(5,1) * 0.9057110466368399 - O(5,3) * 0.4192627457812106 + O(5,5) * 0.0625);
  ...
  c_out[oi++] = (O(6,-5) * -0.3125 + O(6,-3) * 0.8028270361665706 - O(6,-1) * 0.5077524002897476);
  c_out[oi++] = (O(6,1) * 0.4330127018922193 + O(6,3) * 0.6846531968814576 -
              O(6,5) * 0.5863019699779287);
  c_out[oi++] = (O(6,-5) * 0.8028270361665706 - O(6,-3) * 0.0625 - O(6,-1) * 0.5929270612815711);
  c_out[oi++] = (O(6,1) * 0.19764235376052372 + O(6,3) * 0.5625 + O(6,5) * 0.8028270361665706);
  c_out[oi++] = (O(6,-5) * -0.5077524002897476 - O(6,-3) * 0.5929270612815711 -
              O(6,-1) * 0.625);
  c_out[oi++] = (O(6,0) * -0.3125 - O(6,2) * 0.45285552331841994 - O(6,4) * 0.49607837082461076 -
              O(6,6) * 0.6716932893813962);
  c_out[oi++] = (O(6,-6) * -0.879452954966893 - O(6,-4) * 0.4330127018922193 -
              O(6,-2) * 0.19764235376052372);
  c_out[oi++] = (O(6,0) * -0.45285552331841994 - O(6,2) * 0.53125 - O(6,4) * 0.1711632992203644 +
              O(6,6) * 0.6952686081652184);
  c_out[oi++] = (O(6,-6) * 0.46351240544347894 - O(6,-4) * 0.6846531968814576 -
              O(6,-2) * 0.5625);
  c_out[oi++] = (O(6,0) * -0.49607837082461076 - O(6,2) * 0.1711632992203644 +
              O(6,4) * 0.8125 - O(6,6) * 0.2538762001448738);
  c_out[oi++] = (O(6,-6) * -0.10825317547305482 + O(6,-4) * 0.5863019699779287 -
              O(6,-2) * 0.8028270361665706);
  ...
  c_out[oi++] = (O(7,-6)*-0.1875 + O(7,-4)*0.6373774391990981 - O(7,-2)*0.7473912964438374);
  c_out[oi++] = (O(7,0)*-0.47495887979908324 - O(7,2)*0.07328774624724109 +
              O(7,4)*0.78125 - O(7,6)*0.3983608994994363);
  c_out[oi++] = (O(7,-6)*0.6373774391990981 - O(7,-4)*0.5 - O(7,-2)*0.5863019699779287);
  ...
  c_out[oi++] = (O(7,-7)*0.6472598492877494 + O(7,-5)*0.47495887979908324 +
              O(7,-3)*0.42961647140211 + O(7,-1)*0.41339864235384227);
  c_out[oi++] = (O(7,1)*-0.078125 - O(7,3)*0.24356964481437335 - O(7,5)*0.4487939567607835 -
              O(7,7)*0.8562442974262661);
  c_out[oi++] = (O(7,-7)*-0.6991205412874092 + O(7,-5)*0.07328774624724109 +
              O(7,-3)*0.41984465132951254 + O(7,-1)*0.5740991584648073);
  c_out[oi++] = (O(7,1)*-0.24356964481437335 - O(7,3)*0.609375 - O(7,5)*0.5700448858423344 +
              O(7,7)*0.4943528756111367);
  c_out[oi++] = (O(7,-7)*0.2981060004427955 - O(7,-5)*0.78125 - O(7,-3)*0.10364452469860624 +
              O(7,-1)*0.5385527481129402);
  c_out[oi++] = (O(7,1)*-0.4487939567607835 - O(7,3)*0.5700448858423344 + O(7,5)*0.671875 -
              O(7,7)*0.14905300022139775);
  c_out[oi++] = (O(7,-7)*-0.05846339666834283 + O(7,-5)*0.3983608994994363 -
  ...
  c_out[oi++] = (O(9,-9)*-0.016572815184059706 + O(9,-7)*0.1594400908746762 - O(9,-5)*0.509312687906457 +
              O(9,-3)*0.7526807559068452 - O(9,-1)*0.38519665736315783);
  c_out[oi++] = (O(9,1)*0.8171255055356398 - O(9,3)*0.5322256665703469 + O(9,5)*0.21608307321780204 -
              O(9,7)*0.048317644050206957 + O(9,9)*0.00390625);
}

//void SHSwapYZ(RGBSpectrum c_in, RGBSpectrum c_out, int lmax);

void SHConvolveCosTheta(int lmax, Spectrum c_in, Spectrum c_out) {
...
void SHConvolvePhong(int lmax, double n, Spectrum c_in, Spectrum c_out) {
  for (int l = 0; l <= lmax; ++l) {
    double c_phong = Math.exp(-(l * l) / (2.0 * n));
    for (int m = -l; m <= l; ++m) {
      int o = SHIndex(l, m);
      c_out[o] = _lambda(l) * c_in[o] * c_phong;
    }
  }
}

void SHComputeDiffuseTransfer(Point p, Normal n, double rayEpsilon,
    Scene scene, RNG rng, int nSamples, int lmax,
  ...
  for (int i = 0; i < nSamples; ++i) {
    ...
    if (Vector.Dot(w, n) > 0.0 && !scene.intersectP(new Ray(p, w, rayEpsilon))) {
      ...
      for (int j = 0, len = SHTerms(lmax); j < len; ++j) {
        c_transfer[j] += new Spectrum(Ylm[j] * Vector.AbsDot(w, n)) / (pdf * nSamples);
      }
    }
  }
}

void SHComputeTransferMatrix(Point p, double rayEpsilon,
    Scene scene, RNG rng, int nSamples, int lmax, List<Spectrum> T) {
  for (int i = 0, len = SHTerms(lmax) * SHTerms(lmax); i < len; ++i) {
  ...
  for (int i = 0; i < nSamples; ++i) {
    ...
    if (!scene.intersectP(new Ray(p, w, rayEpsilon))) {
      ...
      SHEvaluate(w, lmax, Ylm);
      for (int j = 0, nj = SHTerms(lmax); j < nj; ++j) {
        for (int k = 0, nk = SHTerms(lmax); k < nk; ++k) {
          T[j * SHTerms(lmax) + k] += new Spectrum((Ylm[j] * Ylm[k]) /
                                                   (pdf * nSamples));
        }
      }
    }
...
void SHComputeBSDFMatrix(Spectrum Kd, Spectrum Ks,
    double roughness, RNG rng, int nSamples, int lmax,
    List<Spectrum> B) {
  ...
  Float32List Ylm = new Float32List(SHTerms(lmax) * nSamples);
    ...
    for (int isamp = 0; isamp < nSamples; ++isamp) {
      ...
      if (!f.isBlack()) {
        ...
        for (int i = 0; i < SHTerms(lmax); ++i) {
          for (int j = 0; j < SHTerms(lmax); ++j) {
              B[i * SHTerms(lmax) + j].add(f * (Ylm[isamp * SHTerms(lmax) + j] *
...
void SHMatrixVectorMultiply(List<Spectrum> M, List<Spectrum> v,
                            List<Spectrum> vout, int lmax) {
  for (int i = 0, len = SHTerms(lmax); i < len; ++i) {
    vout[i] = new Spectrum(0.0);
    for (int j = 0; j < SHTerms(lmax); ++j) {
      vout[i] += v[j] * M[SHTerms(lmax) * i + j];
    }
  }
}
...
void _legendrep(double x, int lmax, List<double> out, [int outIndex = 0]) {
  P(int l, int m) {
    int index = (outIndex + SHIndex(l, m)).toInt();
    return out[index];
  }
  ...
  for (int l = 2; l <= lmax; ++l) {
    out[outIndex + SHIndex(l, 0)] = ((2.0 * l - 1.0) * x * P(l - 1, 0) -
                          (l - 1) * P(l - 2, 0)) / l;
    assert(!P(l, 0).isNaN);
  ...
  double xpow = xroot;
  for (int l = 1; l <= lmax; ++l) {
    out[outIndex + SHIndex(l, l)] = neg * dfact * xpow;
    assert(!P(l, l).isNaN);
  ...
  for (int l = 2; l <= lmax; ++l) {
    out[outIndex + SHIndex(l, l - 1)] = x * (2.0 * l - 1.0) * P(l - 1, l - 1);
...
void _sinCosIndexed(double s, double c, int n,
...
void _toZYZ(Matrix4x4 m, List<double> alpha, List<double> beta,
            List<double> gamma) {
